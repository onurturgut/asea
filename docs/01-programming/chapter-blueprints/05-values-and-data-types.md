...
blueprint_id: "V01-C05-BP01"
blueprint_type: "Chapter"
scope_id: "V01-C05"
title: "Chapter 05 Blueprint — Values and Data Types"
version: "0.1.0"
status: "Draft"
...
# Chapter 05 Blueprint — Values and Data Types — Veriyi Temsil Etmek
...
| Primary Lab | `V01-C05-L01` — Classify and Convert Input Values |
| Assessment | `V01-C05-AS01` |
| Next Chapter | `V01-C06` |

### Chapter Purpose

Program verisini value, type ve representation sınırlarıyla modellemek.

### Why This Chapter Exists

This Chapter closes the gap between `V01-C02`, `V01-C04` evidence and the ability to program verisini value, type ve representation sınırlarıyla modellemek. It gives later Chapters a named, traceable capability instead of relying on assumed knowledge.

## Scope

### In Scope

- Plan the mental models, examples, practice, and assessment needed for: Value, primitive type, number, text, Boolean, absence..
- Introduce supporting concepts only where they explain a core mechanism: Representation, literal, conversion, precision, equality..
...
- **V01-LO007:** Örnek değerleri uygun türlere sınıflandırmak.
- **V01-LO008:** type mismatch ve conversion riskini tespit etmek..
...
- **Exit behavior:** The learner can örnek değerleri uygun türlere sınıflandırmak; type mismatch ve conversion riskini tespit etmek..
...
- `V01-LO007` maps to the current Volume 01 source identifier `LO-07`.
- `V01-LO008` maps to the current Volume 01 source identifier `LO-08`.
...
- **Value:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **primitive type:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **number:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **text:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **Boolean:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **absence.:** Plan definition, boundary, mechanism, trace, and one transfer case.

### Supporting Concepts

- Representation: introduce only where it clarifies a core concept or engineering decision.
- literal: introduce only where it clarifies a core concept or engineering decision.
- conversion: introduce only where it clarifies a core concept or engineering decision.
- precision: introduce only where it clarifies a core concept or engineering decision.
...
- Establish Value and primitive type before introducing number, text, Boolean, absence..
- Use supporting concepts after the learner can predict the core behavior.
- End with a transfer task that connects the Chapter capability to `V01-C06`.

### Terminology

- **Value:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **primitive type:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **number:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **text:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **Boolean:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **absence.:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **Representation:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **literal:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **conversion:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **precision:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
...
2. Value mental model
2.1 Definition, purpose, and boundary
2.1.1 Contrast Value with the nearest related concept and identify its observable behavior
2.2 Mechanism and representation
2.2.1 Trace Value through one normal, one boundary, and one failure-oriented case
3. primitive type mental model
3.1 Definition, purpose, and boundary
3.1.1 Contrast primitive type with the nearest related concept and identify its observable behavior
3.2 Mechanism and representation
3.2.1 Trace primitive type through one normal, one boundary, and one failure-oriented case
4. number mental model
4.1 Definition, purpose, and boundary
4.1.1 Contrast number with the nearest related concept and identify its observable behavior
4.2 Mechanism and representation
4.2.1 Trace number through one normal, one boundary, and one failure-oriented case
5. text mental model
5.1 Definition, purpose, and boundary
5.1.1 Contrast text with the nearest related concept and identify its observable behavior
5.2 Mechanism and representation
5.2.1 Trace text through one normal, one boundary, and one failure-oriented case
6. Boolean mental model
6.1 Definition, purpose, and boundary
6.1.1 Contrast Boolean with the nearest related concept and identify its observable behavior
6.2 Mechanism and representation
6.2.1 Trace Boolean through one normal, one boundary, and one failure-oriented case
...
8.2.1 Require measurable evidence aligned with V01-LO007, V01-LO008
...
9.2.1 Connect retained knowledge to V01-C06

### Planned Diagrams

| Diagram Name | Purpose | Diagram Type | Concepts Covered |
|---|---|---|---|
| Values and Data Types Concept Boundary | Show the Chapter's main concepts and exclusions | Concept map | Value, primitive type, number |
| Value Behavior Flow | Make state, decision, or transformation order visible | Flow diagram | Value, primitive type, number, text |
| Dependency to Evidence | Connect prerequisite knowledge to assessment evidence | Traceability flow | V01-C02, V01-C04 → V01-LO007, V01-LO008 |
...
|---|---|---|---|---|---|
| `V01-C05-CODE01` | Demonstrate the smallest observable behavior | Value | Guided | Deterministic result with an explained trace | Görünüşü aynı değerleri aynı tür sanmak |
| `V01-C05-CODE02` | Expose a boundary or invalid case | primitive type | Supported | Controlled boundary behavior and diagnostic evidence | conversion kaybını göz ardı etmek. |
| `V01-C05-CODE03` | Compare two valid designs | absence. | Transfer | Same required behavior with a justified trade-off | Choosing by syntax preference without evidence |
...
- **Repair example:** Learner identifies the smallest incorrect assumption behind “Görünüşü aynı değerleri aynı tür sanmak”.
- **Transfer example:** Learner applies the Chapter model to a new domain and explains which constraints changed.

### Common Mistakes

- Plan a symptom → cause → impact → diagnosis sequence for: Görünüşü aynı değerleri aynı tür sanmak.
- Plan a symptom → cause → impact → diagnosis sequence for: conversion kaybını göz ardı etmek..

### Common Misconceptions

- Knowing the syntax is equivalent to demonstrating `V01-LO007`.
...
- Plan rationale, boundary, and counterexample for: Tür beklentisini açık yazmak.
- Plan rationale, boundary, and counterexample for: conversion'ı boundary'de doğrulamak..
...
- **Lab ID:** `V01-C05-L01`
- **Objectives:** Değerleri türlerine ayırmak ve güvenli conversion sınırlarını belirlemek.
- **Scenario:** Verilen değer setini sınıflandırmak; conversion tablosu oluşturmak; kayıplı dönüşümleri işaretlemek.
- **Skills Measured:** Algorithm input ve trace bilgisi.
- **Estimated Time:** 60-75 minutes
- **Source contract:** [Laboratory Blueprint](../labs.md#lab-05---classify-and-convert-input-values)

The Lab file is not created in this task.

### Planned Independent Exercise

- **Exercise ID:** `V01-C05-EX01`
- **Objective:** Produce one inspectable artifact that demonstrates V01-LO007 and V01-LO008 before the Lab.
- **Evidence:** A trace, decision record, model, or test set appropriate to Value.
- **Constraint:** 20-30 minutes, one normal case, one boundary case, and a self-check against the Chapter contract.

### Planned Challenge

- **Challenge ID:** `V01-C05-CH01`
- **Challenge:** Detect Lossy Conversions.
...
- **Quiz ID:** `V01-C05-QZ01`
- **Scope:** Value, primitive type, number, text, Boolean, absence.; common mistakes; prerequisite-to-output reasoning.
- **Learning Outcomes:** `V01-LO007`, `V01-LO008`
...
- **Interview ID:** `V01-C05-IV01`
- **Primary focus:** Tür bilgisi hangi hata sınıflarını önler?
...
- Plan one prompt requiring a dated next action for retaining Value.
...
|---|---|---|---|
| Chapter Assessment | `V01-C05-AS01` | `V01-LO007`, `V01-LO008` | Quiz, open-ended response, Lab, and reflection |
| Quiz | `V01-C05-QZ01` | `V01-LO007`, `V01-LO008` | Concept and trace evidence |
| Lab | `V01-C05-L01` | `V01-LO007`, `V01-LO008` | Applied artifact and explanation |
| Challenge | `V01-C05-CH01` | `V01-LO007`, `V01-LO008` | Independent transfer evidence |
| Interview | `V01-C05-IV01` | `V01-LO007`, `V01-LO008` | Reasoning and communication evidence |

### Project Relationship

- **Mini Project:** V01-P02 — Data Transformation Console; this Chapter supplies the program verisini value, type ve representation sınırlarıyla modellemek capability.
- **Capstone:** V01-CP01 — Reliable Command-Line Problem Solver; contribution: explicit value and type choices in the data dictionary.
...
- `V01-C05` supplies assessed evidence to `V01-C06` and later dependent Chapters.

## Traceability

| Source Outcome | v2 Outcome | Chapter | Lab | Assessment | Project |
|---|---|---|---|---|---|
| `LO-07` | `V01-LO007` | `V01-C05` | `V01-C05-L01` | `V01-C05-AS01` | `V01-P02` |
| `LO-08` | `V01-LO008` | `V01-C05` | `V01-C05-L01` | `V01-C05-AS01` | `V01-P02` |
