...
- `V01-C31` must be demonstrated before this Chapter is promoted to Stable.
  Numeric production order currently places C31 later; C22 may be authored and
  piloted with arrays, loops, conditions, and functions while this dependency
...
- `V01-C31` supplies collection-transformation evidence required by the canonical
  registry. It is an open production-order dependency, not assumed knowledge.
