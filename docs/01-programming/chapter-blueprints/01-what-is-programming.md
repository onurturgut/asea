...
blueprint_id: "V01-C01-BP01"
blueprint_type: "Chapter"
scope_id: "V01-C01"
title: "Chapter 01 Blueprint — What Is Programming?"
version: "0.1.0"
status: "Draft"
...
# Chapter 01 Blueprint — What Is Programming? — Programlama ve Hesaplama
...
| Primary Lab | `V01-C01-L01` — Human Instruction Interpreter |
| Assessment | `V01-C01-AS01` |
| Next Chapter | `V01-C02` |

### Chapter Purpose

Program, algorithm, instruction ve computation kavramlarını ayırmak; programlamayı problem çözme aracı olarak konumlandırmak.

### Why This Chapter Exists

This Chapter closes the gap between `V00` evidence and the ability to program, algorithm, instruction ve computation kavramlarını ayırmak. It gives later Chapters a named, traceable capability instead of relying on assumed knowledge.

## Scope

### In Scope

- Plan the mental models, examples, practice, and assessment needed for: Program, instruction, computation, input, process, output..
- Introduce supporting concepts only where they explain a core mechanism: Determinism, automation, correctness, programming language..
...
- `V00`: required evidence defined by the dependency map.
- Volume 00 study, evidence, and reflection discipline.
- Terminology already established by prerequisite Chapters.

### Knowledge Outputs

- **V01-LO001:** Program ile algoritma farkını örnekle açıklamak.
- **V01-LO002:** bir günlük süreci kesin talimatlara dönüştürmek..
...
- **Exit behavior:** The learner can program ile algoritma farkını örnekle açıklamak; bir günlük süreci kesin talimatlara dönüştürmek..
...
- `V00` must be demonstrated through its assessment gate before this Chapter begins.
...
- `V01-LO001` maps to the current Volume 01 source identifier `LO-01`.
- `V01-LO002` maps to the current Volume 01 source identifier `LO-02`.
...
- **Program:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **instruction:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **computation:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **input:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **process:** Plan definition, boundary, mechanism, trace, and one transfer case.
- **output.:** Plan definition, boundary, mechanism, trace, and one transfer case.

### Supporting Concepts

- Determinism: introduce only where it clarifies a core concept or engineering decision.
- automation: introduce only where it clarifies a core concept or engineering decision.
- correctness: introduce only where it clarifies a core concept or engineering decision.
- programming language.: introduce only where it clarifies a core concept or engineering decision.

### Concept Dependency

- Begin with the prerequisite evidence and make the new conceptual gap explicit.
- Establish Program and instruction before introducing computation, input, process, output..
- Use supporting concepts after the learner can predict the core behavior.
- End with a transfer task that connects the Chapter capability to `V01-C02`.

### Terminology

- **Program:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **instruction:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **computation:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
...
- **Determinism:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **automation:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **correctness:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
- **programming language.:** Plan the first-use bilingual definition, one non-example, and consistent later usage.
...
2. Program mental model
2.1 Definition, purpose, and boundary
2.1.1 Contrast Program with the nearest related concept and identify its observable behavior
2.2 Mechanism and representation
2.2.1 Trace Program through one normal, one boundary, and one failure-oriented case
3. instruction mental model
3.1 Definition, purpose, and boundary
3.1.1 Contrast instruction with the nearest related concept and identify its observable behavior
3.2 Mechanism and representation
3.2.1 Trace instruction through one normal, one boundary, and one failure-oriented case
4. computation mental model
4.1 Definition, purpose, and boundary
4.1.1 Contrast computation with the nearest related concept and identify its observable behavior
4.2 Mechanism and representation
4.2.1 Trace computation through one normal, one boundary, and one failure-oriented case
5. input mental model
5.1 Definition, purpose, and boundary
5.1.1 Contrast input with the nearest related concept and identify its observable behavior
5.2 Mechanism and representation
5.2.1 Trace input through one normal, one boundary, and one failure-oriented case
6. process mental model
6.1 Definition, purpose, and boundary
6.1.1 Contrast process with the nearest related concept and identify its observable behavior
6.2 Mechanism and representation
6.2.1 Trace process through one normal, one boundary, and one failure-oriented case
...
8.2.1 Require measurable evidence aligned with V01-LO001, V01-LO002
...
9.2.1 Connect retained knowledge to V01-C02

### Planned Diagrams

| Diagram Name | Purpose | Diagram Type | Concepts Covered |
|---|---|---|---|
| What Is Programming? Concept Boundary | Show the Chapter's main concepts and exclusions | Concept map | Program, instruction, computation |
| Program Behavior Flow | Make state, decision, or transformation order visible | Flow diagram | Program, instruction, computation, input |
| Dependency to Evidence | Connect prerequisite knowledge to assessment evidence | Traceability flow | V00 → V01-LO001, V01-LO002 |
...
|---|---|---|---|---|---|
| `V01-C01-CODE01` | Demonstrate the smallest observable behavior | Program | Guided | Deterministic result with an explained trace | Programlamayı sözdizimi ezberi sanmak |
| `V01-C01-CODE02` | Expose a boundary or invalid case | instruction | Supported | Controlled boundary behavior and diagnostic evidence | belirsiz talimatı yürütülebilir kabul etmek. |
| `V01-C01-CODE03` | Compare two valid designs | output. | Transfer | Same required behavior with a justified trade-off | Choosing by syntax preference without evidence |
...
- **Repair example:** Learner identifies the smallest incorrect assumption behind “Programlamayı sözdizimi ezberi sanmak”.
- **Transfer example:** Learner applies the Chapter model to a new domain and explains which constraints changed.

### Common Mistakes

- Plan a symptom → cause → impact → diagnosis sequence for: Programlamayı sözdizimi ezberi sanmak.
- Plan a symptom → cause → impact → diagnosis sequence for: belirsiz talimatı yürütülebilir kabul etmek..

### Common Misconceptions

- Knowing the syntax is equivalent to demonstrating `V01-LO001`.
...
- Plan rationale, boundary, and counterexample for: Önce beklenen sonucu tanımlamak.
- Plan rationale, boundary, and counterexample for: talimatları test edilebilir yazmak..
...
- **Lab ID:** `V01-C01-L01`
- **Objectives:** Belirsiz doğal dil talimatlarını yürütülebilir adımlara dönüştürmek.
- **Scenario:** Bir günlük sürecin girdisini ve beklenen sonucunu tanımlamak; talimatları sıralamak; iki belirsizliği test girdisiyle göstermek.
- **Skills Measured:** Program, instruction, input, output ayrımı.
- **Estimated Time:** 60-75 minutes
- **Source contract:** [Laboratory Blueprint](../labs.md#lab-01---human-instruction-interpreter)

The Lab file is not created in this task.

### Planned Independent Exercise

- **Exercise ID:** `V01-C01-EX01`
- **Objective:** Produce one inspectable artifact that demonstrates V01-LO001 and V01-LO002 before the Lab.
- **Evidence:** A trace, decision record, model, or test set appropriate to Program.
- **Constraint:** 20-30 minutes, one normal case, one boundary case, and a self-check against the Chapter contract.

### Planned Challenge

- **Challenge ID:** `V01-C01-CH01`
- **Challenge:** Ambiguous Recipe Repair.
...
- **Quiz ID:** `V01-C01-QZ01`
- **Scope:** Program, instruction, computation, input, process, output.; common mistakes; prerequisite-to-output reasoning.
...
- **Interview ID:** `V01-C01-IV01`
- **Primary focus:** Program ve algoritma ayrımını iki dakikada açıklama.
...
- Plan one prompt requiring a dated next action for retaining Program.
...
|---|---|---|---|
| Chapter Assessment | `V01-C01-AS01` | `V01-LO001`, `V01-LO002` | Quiz, open-ended response, Lab, and reflection |
| Quiz | `V01-C01-QZ01` | `V01-LO001`, `V01-LO002` | Concept and trace evidence |
| Lab | `V01-C01-L01` | `V01-LO001`, `V01-LO002` | Applied artifact and explanation |
| Challenge | `V01-C01-CH01` | `V01-LO001`, `V01-LO002` | Independent transfer evidence |
| Interview | `V01-C01-IV01` | `V01-LO001`, `V01-LO002` | Reasoning and communication evidence |

### Project Relationship

- **Mini Project:** V01-P01 — Instruction Simulator; this Chapter supplies the program, algorithm, instruction ve computation kavramlarını ayırmak capability.
- **Capstone:** V01-CP01 — Reliable Command-Line Problem Solver; contribution: precise instruction and computation vocabulary for requirement interpretation.

## Dependencies

### Incoming Dependencies

- `V00` supplies required knowledge or assessed evidence.
...
- `V01-C01` supplies assessed evidence to `V01-C02` and later dependent Chapters.

## Traceability

| Source Outcome | v2 Outcome | Chapter | Lab | Assessment | Project |
|---|---|---|---|---|---|
| `LO-01` | `V01-LO001` | `V01-C01` | `V01-C01-L01` | `V01-C01-AS01` | `V01-P01` |
| `LO-02` | `V01-LO002` | `V01-C01` | `V01-C01-L01` | `V01-C01-AS01` | `V01-P01` |
